...
def convert_schema_to_model(api: Api, mschema: Schema, name: str='') -> Model:
    ...
    if isinstance(mschema, Schema) and not mschema == type(Schema):
        m_fields = mschema.declared_fields
    elif mschema == type(Schema):
        m_fields = mschema._declared_fields
    elif issubclass(mschema, BaseSchema.Meta) or mschema.Meta == BaseSchema.Meta:
        m_fields = mschema._declared_fields
    else:
        return api.model(name, mschema)
